                                        ...
                                        switch (databaseType) {
                                                ...
                                                case "sqlite": {
                                                        ...
                                                        if (databaseType === "sqlite") {
                                                                const [instance, created] = await threadModel.findOrCreate({
                                                                        where: { threadID: threadData.threadID },
                                                                        defaults: threadData
                                                                });
                                                                dataCreated = instance.get({ plain: true });
                                                                if (!created) {
                                                                        const existingIndex = global.db.allThreadData.findIndex(t => t.threadID == threadData.threadID);
                                                                        if (existingIndex === -1) {
