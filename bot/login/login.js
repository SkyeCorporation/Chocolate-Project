...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                ...
                login({ appState }, global.GoatBot.config.optionsFca, async function (error, api) {
                        ...
                        // ———————————————— SECOND ACCOUNT ———————————————— //
                        const dirAccount2 = path.normalize(`${process.cwd()}/account2.txt`);
                        if (!process.env.BOT_INSTANCE && existsSync(dirAccount2)) {
                                try {
                                        const account2Text = readFileSync(dirAccount2, "utf8").trim();
                                        if (account2Text && account2Text !== "[]" && account2Text !== "") {
                                                let appState2 = JSON.parse(account2Text);
                                                if (Array.isArray(appState2) && appState2.length > 0) {
                                                        appState2 = appState2.map(item => ({
                                                                ...item,
                                                                domain: "facebook.com",
                                                                path: "/",
                                                                hostOnly: false,
                                                        ...
                                                        login({ appState: appState2 }, global.GoatBot.config.optionsFca, async function (error2, api2) {
                                                                if (error2) {
                                                                        log.err("LOGIN ACCOUNT 2", "Failed to login with account2.txt", error2);
                                                                        return;
                                                                }
                                                                const botID2 = api2.getCurrentUserID();
                                                                global.GoatBot.secondBotID = botID2;
                                                                log.info("LOGIN ACCOUNT 2", `Second account logged in: ${botID2}`);
                                                                const handlerAction2 = require("../handler/handlerAction.js")(api2, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData);
                                                                global.GoatBot.Listening2 = api2.listenMqtt(async function (err2, event2) {
                                                                        if (err2) {
                                                                                log.err("ACCOUNT 2 LISTEN", "Listen error", err2);
                                                                                return;
                                                                        }
                                                                        handlerAction2(event2);
                                                                });
                                                                log.info("ACCOUNT 2", "Second account is now listening for messages");
                                                        });
                                                }
                                        }
                                } catch (e) {
                                        log.warn("LOGIN ACCOUNT 2", "Could not start second account: " + e.message);
                                }
                        }
